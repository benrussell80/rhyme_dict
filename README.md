...
pronunciation. Using the [cmudict](https://github.com/prosegrinder/python-cmudict) python package I acquired each pronunciation for
...
the latter is stressed then unstressed (SUBject). Stresses are denoted by zeros and ones.
...
From this, we can start creating rhyme patterns, like "EH1 K T" for each of these words. So if we come across a new word, we can compare its rhyme pattern to those rhyme patterns
that we have already catalogued and pair them up.

### Slant Rhymes
A step beyond these quantification is the concept of a slant rhyme. Generally, a slant rhyme consists of two words who almost rhyme but have some minor difference.
For example, the words "steak" and "lace" share a common vowel sound but miss each other on the final consonants. Looking at their pronunciations we get something
very interesting:

    steak:      "S T EY1 K"
    lace:         "L EY1 S"

Their rhyme patterns ("EY1 K" and "EY1 S") are only different by one phoneme. Let's run with this. Let's use the Levenshtein distance as a way of quantifying
...
[Neo4j](https://neo4j.com/) is a graph database that stores data in the form of nodes and relationships. I created a database that stores words and rhyme patterns as nodes and rhymes and slant rhymes as relationships between those nodes. The production database for this project is hosted with AWS EC2.
...
### Development
I like to think this project is intermediate-friendly since it isn't too large to contribute to while still using a variety of technologies. A lot can be added to this project such as:
...
Thanks so much for checking the project out!!
